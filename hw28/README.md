## PostgreSQL, репликация, резервное копирование.
...
Разумной стратегией для нормальных рабочих нагрузок может быть полное резервное копирование каждую неделю с последующим ежедневным добавочным резервным копированием. `repo1-retention-full=1` означает, что за один раз будет храниться только одна полная резервная копия. Срок действия инкрементных резервных копий истекает только после истечения срока их полной или дифференциальной резервной копии. Настройка cron на сервере резервного копирования требует минимальных усилий для создания еженедельной полной резервной копии и ежедневной инкрементной.
