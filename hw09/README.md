...
Что, из каких файлов в `/proc` мы должны взять, какие оттуда данные прочитать, мы можем узнать из документа [man7](http://man7.org/linux/man-pages/man5/proc.5.html). Это файлы `/proc/$pid/stat` ,`/proc/$pid/cmdline`, `/proc/$pid/environ`, `/proc/$pid/smaps` и папка, содержащая файловые дескрипторы процесса `/proc/$pid/fd`.
...
После всех преобразований составляем вывод, близкий к выводу `ps`. См. файл [prcss.sh](https://github.com/kakoka/otus-homework/blob/master/hw09/prcss.sh). Файл нужно запустить от рута - `sudo chmod +x prcss.sh && sudo ./prcss.sh`. 
...
Первые две запускают утилиту `dd`, которая генерирует 1Gb файлы используя `/dev/random` с разным ionice, вторые две сжимают получившиеся файлы посредством `gzip` в архив с максимальной степенью компрессии. Далее, запустим параллельно первые две функции, потом вторые.
